...
const SidebarNoteItem: React.FC<SidebarNoteItemProps> = ({
        ...
        if (note.hasChildren && children.length === 0 && !hasAttemptedLoad) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    fetchedChildren.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
    ...
    // Aligned base indentation: 7px base + level * 12px
    ...
    return (
        <div className="sidebar-tree-item-container">
            <div
                ...
                style={{ paddingLeft: `${7 + indentation}px` }}
            >
                {/* Icon Slot */}
                <div
                ...
                >
                    ...
                    {(note.hasChildren || children.length > 0) && (
                        <div className="icon-chevron" style={{ transform: isExpanded ? 'rotate(90deg)' : 'rotate(0deg)' }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                {isEditing ? (
                    ...
                    <div className="item-title" style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={note.title}>
                        {note.title || 'Untitled'}
                    </div>
                )}
            ...
            {isExpanded && (
                <div className="sidebar-tree-children">
                    {isLoadingChildren ? (
                        <div style={{ paddingLeft: `${32 + indentation}px`, paddingTop: '4px', paddingBottom: '4px', color: 'var(--text-tertiary)', fontSize: '0.8rem' }}>
                            Loading...
                        </div>
                    ) : (
                        children.map(child => (
