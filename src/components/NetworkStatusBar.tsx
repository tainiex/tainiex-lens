...
const NetworkStatusBar = ({ connectionState: propsState, onReconnect: propsOnReconnect }: NetworkStatusBarProps = {}) => {
  ...
  const getStatusConfig = () => {
    switch (status) {
      case 'connecting':
        return {
          icon: (
            <svg className="status-icon spinning" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div className={`network-status-indicator-global ${config.className}`} title={config.title}>
      ...
      */}
      {(status === 'disconnected' || status === 'failed') && onReconnect && (
        <div className="reconnect-overlay" onClick={onReconnect} title="Click to Reconnect" />
      )}
    </div>
  );
};
