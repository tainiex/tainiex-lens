...
const AppLayout = () => {
    ...
    const getCurrentId = () => {
        ...
        // /app/ID or /app/notes/ID
        if (isNotesPath && parts.length > 3) return parts[3]; // /app/notes/123
        if (!isNotesPath && parts.length > 2 && parts[2] !== '') return parts[2]; // /app/123
        return null;
    };
    ...
    const handleDeleteNote = useCallback(async (id: string) => {
        try {
            ...
            if (res.ok) {
                ...
                if (window.location.pathname.includes(id)) {
                    navigate('/app/notes', { state: { sidebarOpen: isSidebarOpenRef.current } });
                }
            }
        } catch (error) {
