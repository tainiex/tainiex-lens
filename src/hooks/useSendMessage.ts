    ...
    return new Promise((resolve, reject) => {
      ...
      streamTimeoutRef.current = setTimeout(() => {
        if (isStreamingRef.current) {
          const timeoutError = ErrorHandler.parseError(new Error(`Message stream timeout - Server did not respond within ${streamTimeoutMs / 1000} seconds.`), 'send message');
          cleanup();
          reject(timeoutError);
        }
      }, streamTimeoutMs);
      ...
      const handleStream = (event: unknown) => {
        ...
        if (validatedEvent.type === 'chunk') {
          ...
          setCurrentMessage(prev => prev ? { ...prev, lastActivity: Date.now() } : null);
      ...
      const handleError = (error: any) => {
        ...
        const apiError = ErrorHandler.parseError(new Error(errorMessage), 'chat stream');
        reject(apiError);
      };
      ...
      try {
        socket.emit('chat:send', validatedPayload, (ack: any) => {
          if (ack && ack.error) {
            ...
            reject(ErrorHandler.parseError(new Error(ackError), 'message acknowledgment'));
          }
        });
      } catch (error) {
        cleanup();
