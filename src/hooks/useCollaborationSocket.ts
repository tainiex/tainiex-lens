...
export function useCollaborationSocket(
  ...
  const [connectionState, setConnectionState] = useState<CollaborationConnectionState>({ status: 'disconnected', noteId: null });
  ...
  useEffect(() => {
    ...
    const initSocket = async () => {
      ...
      updateConnectionState(activeSocket.connected ? 'connected' : 'connecting', currentNoteIdRef.current);
            ...
            if (pendingUpdatesQueueRef.current.length > 0) {
              pendingUpdatesQueueRef.current.forEach(({ update, targetNoteId }) => {
                if (targetNoteId === payload.noteId) {
                  ...
                  // @ts-ignore
                  activeSocket?.emit('yjs:update', { noteId: targetNoteId, update: encodedUpdate });
                }
              });
              pendingUpdatesQueueRef.current = [];
